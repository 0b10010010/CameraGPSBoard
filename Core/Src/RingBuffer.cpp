...
bool RingBuffer::set_size(uint32_t _size)
{
    head = tail = 0;
    if (_size != size) {
        free(buf);
...
uint32_t RingBuffer::available(void) const
{
	uint32_t _tail = tail;
	if (head > _tail) {
...
uint32_t RingBuffer::space(void) const
{
    if (size == 0) {
        return 0;
    }
...
uint32_t RingBuffer::write(const uint8_t *data, uint32_t len)
{
	RingBuffer::IoVec vec[2];
...
/*
  ...
  popping it
 */
bool RingBuffer::update(const uint8_t *data, uint32_t len)
{
    if (len > available()) {
        return false;
    }
...
bool RingBuffer::advance(uint32_t n)
{
    if (n > available()) {
        return false;
    }
...
uint8_t RingBuffer::peekiovec(RingBuffer::IoVec iovec[2], uint32_t len)
{
 ...
 */
uint32_t RingBuffer::peekbytes(uint8_t *data, uint32_t len)
{
    RingBuffer::IoVec vec[2];
...
uint8_t RingBuffer::reserve(RingBuffer::IoVec iovec[2], uint32_t len)
{
    uint32_t n = space();
 ...
 */
bool RingBuffer::commit(uint32_t len)
{
    if (len > space()) {
 ...
 */
const uint8_t *RingBuffer::readptr(uint32_t &available_bytes)
{
    uint32_t _tail = tail;
...
int16_t RingBuffer::peek(uint32_t ofs) const
{
    if (ofs >= available()) {
        return -1;
    }
